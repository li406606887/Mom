...
@implementation ModifyPayPwdView
-(instancetype)initWithViewModel:(id<BaseViewModelProtocol>)viewModel {
    self.viewModel = (ModifyPayPwdViewModel *)viewModel;
...
-(void)updateConstraints {
    ...
    [self.pwdField mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self);
        make.top.equalTo(self.oldPwdField.mas_bottom).with.offset(10);
        make.size.mas_offset(CGSizeMake(SCREEN_WIDTH-30, 44));
    }];
...
- (UITextField *)oldPwdField {
    if (!_oldPwdField) {
        _oldPwdField = [self creatTextFieldWithPlaceholder:@"旧密码"];
    }
    return _oldPwdField;
}
...
- (UITextField *)pwdField {
    if (!_pwdField) {
        _pwdField = [self creatTextFieldWithPlaceholder:@"新支付密码(6数字"];
    }
    return _pwdField;
}
...
- (UIButton *)login {
    if (!_login) {
        ...
        [[_login rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSDictionary *param = @{@"oldPayPassword":self.oldPwdField.text,@"payPassword":self.surePwdField.text};
            [self.viewModel.modifyPasswordCommand execute:param];
        }];
    }
    return _login;
}
