...
@implementation SetingPayPwdView
-(instancetype)initWithViewModel:(id<BaseViewModelProtocol>)viewModel {
    self.viewModel = (SetingPayPwdViewModel *)viewModel;
...
- (UITextField *)pwdField {
    if (!_pwdField) {
        _pwdField = [self creatTextFieldWithPlaceholder:@"新支付密码(6位数)"];
    }
    return _pwdField;
}
...
- (UITextField *)surePwdField {
    if (!_surePwdField) {
        _surePwdField = [self creatTextFieldWithPlaceholder:@"请再次输入新支付密码"];
    }
...
- (UIButton *)login {
    if (!_login) {
        ...
        [_login setTitle:@"保存支付密码" forState:UIControlStateNormal];
        ...
        [[_login rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSDictionary *param = @{@"oldPayPassword":@"",@"payPassword":self.surePwdField.text};
