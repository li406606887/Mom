...
- (instancetype)initWithViewModel:(id<BaseViewModelProtocol>)viewModel {
    self.viewModel = (BindPhoneViewModel*)viewModel;
...
- (UILabel *)validationBtn {
    if (!_validationBtn) {
        ...
        [[tap rac_gestureSignal] subscribeNext:^(__kindof UIGestureRecognizer * _Nullable x) {
            ...
            [self.viewModel.getCodeCommand execute:@{@"phone":self.phone.text,@"type":@"4"}];
        }];
...
- (UIButton *)login {
    if (!_login) {
        ...
        [[_login rac_signalForControlEvents:UIControlEventTouchUpInside] subscribeNext:^(__kindof UIControl * _Nullable x) {
            ...
            NSDictionary *param = @{@"type":@"2",@"wxcode":self.viewModel.wxcode,@"code":self.validationField.text,@"phone":self.phone.text,@"openid":self.viewModel.openId};
