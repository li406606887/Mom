...
-(void)btnClick {
    ...
    [self.viewModel.rechargeSubject sendNext:self.moneyTextfiled.text];
}
-(void)setUserModel:(UserDataModel *)userModel {
    if (userModel) {
        self.balanceLabel.text = [NSString stringWithFormat:@"当前余额 %@",userModel.balance];
    }
}
-(void)setupViews {
